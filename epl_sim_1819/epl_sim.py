...
table_csv = pd.read_ctable_csv = pd.read_csv('epl-table.csv')
...
for i in range(remaining_matches['Date'].count()):
    ...
    poi_home = rd.poisson(exg_home, poi_count)
    poi_away = rd.poisson(exg_away, poi_count)

    strA = (str(i) + 'a')
    strB = (str(i) + 'b')
    poisson[strA] = poi_home
    poisson[strB] = poi_away
...
for j in range(poi_count):
    ...
    for i in range(remaining_matches['Date'].count()):
        ...
        rd_poi_home = poisson[strA][j]
        rd_poi_away = poisson[strB][j]
        score = str(rd_poi_home) + ' - ' + str(rd_poi_away)
        remaining_matches['Result'].iloc[i] = score

        home_pts, away_pts = get_points(score)

        update_table(home_team, home_pts, rd_poi_home, rd_poi_away, False, table_csv)
        update_table(away_team, away_pts, rd_poi_away, rd_poi_home, True, table_csv)

        file_path = './Results/' + str(j) + '/epl-table' + str(i) + '.csv'
        table_csv = sort_table(table_csv)
        try:
    ...
    table_csv.to_csv('./Results/' + str(j) + '/epl-table-final.csv')
